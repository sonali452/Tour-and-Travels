						...
						foreach ( $terms_by_ids as $term_id => $term_object ) {
							...
							<div class="item wpte-trip-category">
								<address
									itemprop="address"
									style="display: none;"
								><?php echo esc_html( $term_object->name ); ?></address>
								<div class="wpte-trip-category-img-wrap">
									...
									<div class="wpte-trip-category-overlay">
										<div class="wpte-trip-subcat-wrap">
											...
											if ( count( $term_object->children ) > 0 ) :
												foreach ( $term_object->children as $index => $child_term_id ) {
													...
													printf( '<a href="%1$s">%2$s</a>', $terms_by_ids[ $child_term_id ]->link, $terms_by_ids[ $child_term_id ]->name );
										...
										<div class="wpte-trip-category-btn">
											<?php printf( '<a href="%1$s" class="wpte-trip-cat-btn">%2$s</a>', $term_object->link, __( 'View All', 'wp-travel-engine' ) ); ?>
										</div>
									</div>
								</div>
								<div class="wpte-trip-category-text-wrap">
									<h2 class="wpte-trip-category-title" itemprop="name">
										<a href="<?php echo esc_url( $term_object->link ); ?>">
										<?php echo esc_attr( $term_object->name ); ?></a><span class="trip-count"><?php printf( _n( '(%d Trip)', '(%d Trips)', $term_object->count, 'wp-travel-engine' ), $term_object->count ); ?></span>
									</h2>
								</div>
							</div>
							<?php
