				...
				<div class="wpte-tab-content wte-addons-content current">
					...
					foreach ( $licensed_addons as $key => $addon ) {
						...
						if ( $license_status->license === 'valid' ) {
							...
							$activation_message = isset( $_GET['wte_license_error_msg'] ) && $_GET['wte_addon_name'] === $addon->slug ? $_GET['wte_license_error_msg'] : sprintf( __( 'Your license key for %1$s addon is not activated on this site yet. Please activate.', 'wp-travel-engine' ), $addon->title );
							$msg_color          = 'style="color:#f66757"';
						}
						?>
							<div class="wpte-floated <?php echo esc_attr( $active_class ); ?>">
								<label for="wp_travel_engine_license[<?php echo $addon->slug; ?>_license_key]" class="wpte-field-label"><?php echo esc_html( $addon->title ); ?></label>
								<div class="wpte-field wpte-password">
									<input id="<?php echo $addon->slug; ?>" class="wp_travel_engine_addon_license_key" name="wp_travel_engine_license[<?php echo $addon->slug; ?>_license_key]" type="text" class="regular-text" value="<?php echo $addon->license_key; ?>" />
								...
								<?php if ( $license_status->license == 'valid' ) { ?>
									<input type="submit" class="wpte-btn wpte-btn-deactive deactivate-license" data-id="<?php echo $addon->slug; ?>" name="edd_license_deactivate" value="<?php echo 'Deactivate License'; ?>"/>
								<?php } elseif ( ! empty( $addon->{'license_key'} ) ) { ?>
									<input type="submit" class="wpte-btn wpte-btn-active activate-license" data-id="<?php echo $addon->slug; ?>" name="edd_license_activate" value="<?php echo 'Activate License'; ?>"/>
								<?php } ?>
								</div>
							</div>
						<?php } ?>
