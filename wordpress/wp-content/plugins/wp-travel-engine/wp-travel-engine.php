...
define( 'WP_TRAVEL_ENGINE_VERSION', '5.2.2' );
		...
		function() {
			echo wp_kses_post(
				sprintf(
					...
					esc_html__( sprintf( 'The PHP version doesn\'t meet requirement of WP Travel Engine, plugin is currently NOT RUNNING.', 'wp-travel-engine' ) )
				)
			);
		}
	);
	...
	add_action(
		...
		function() {
			echo wp_kses_post(
				sprintf(
					'<div class="error">%1$s</div>',
					esc_html__( sprintf( 'The WordPress version is earlier than the minimum requirement to run WP Travel Engine, the plugin is NOT RUNNING.', 'wp-travel-engine' ) )
				)
			);
		}
	);
