...
function wte_get_dates_meta_for_pricing( $fsds ) {
	...
	if ( isset( $fsds['departure_dates'] ) && ! empty( $fsds['departure_dates']['sdate'] ) ) {
		...
		foreach ( $keys as $key ) {
			...
			$recurring_obj = array(
				'enable'      => isset( $recurring['enable'] ) && $recurring['enable'] == 'true',
			...
			$dates[] = array(
				...
				'is_recurring'       => isset( $recurring['enable'] ) && $recurring['enable'] == 'true',
				'rrule'              => $recurring_obj,
...
function migrate_trip_dates_and_pricings( $trip, $force = false ) {
	...
	$trip_metas = get_post_meta( $trip->ID );
	...
	$enabled_multiple_category_pricing = empty( $fsds ) || ( isset( $fsds['enable_multiple_pricing_fsd'] ) && '1' == $fsds['enable_multiple_pricing_fsd'] );
