...
class WTE_Booking_Emails extends WP_Travel_Engine_Emails {
	...
	public static function sample_template_preview( $email_template_type, $to ) {
		...
		wte_get_template( 'emails/email-header.php' );
		echo self::get_template_content( $email_template_type, '', $to );
		wte_get_template( 'emails/email-footer.php' );
		exit;
	}
		...
		if ( is_array( $order_trips ) ) :
			...
			foreach ( $order_trips as $trip ) :
				...
				<table width="100%" cellpadding="0" cellspacing="0">
					<tr>
						<td colspan="2"><b><?php echo $trip->title; ?></b></td>
					</tr>
					<tr>
						...
						<td class="alignright">
							<table width="100%" cellpadding="0" cellspacing="0">
								...
								foreach ( $trip->pax as $pricing_category_id => $tcount ) {
									...
									echo '<tr>';
									echo "<td class=\"alignright\">{$label}</td>";
									echo "<td>{$tcount} X " . wte_get_formated_price( $pax_cost, $currency, '', ! 0 ) . ' = ' . wte_get_formated_price( $trip->pax_cost[ $pricing_category_id ], $currency, '', ! 0 ) . '</td>';
									echo '</tr>';
								}
								?>
								<tr>
									<td width="50%"><?php esc_html_e( 'Subtotal', 'wp-travel-engine' ); ?></td>
									<td width="50%"><?php echo wte_get_formated_price( +$sum, $currency, '', ! 0 ); ?></td>
								</tr>
							</table>
						</td>
					</tr>
						...
						<td class="alignright">
							<table width="100%" cellpadding="0" cellspacing="0">
								...
								foreach ( $trip->trip_extras as $index => $tx ) {
									...
									$sum     += $tx_total;
									echo '<tr>';
									echo "<td>{$tx['extra_service']}</td>";
									echo "<td>{$tx['qty']} X " . wte_get_formated_price( +$tx['price'], $currency, '', ! 0 ) . ' = ' . wte_get_formated_price( +$tx_total, $currency, '', ! 0 ) . '</td>';
									echo '</tr>';
								}
								?>
								<tr>
									<td width="50%"><?php esc_html_e( 'Subtotal', 'wp-travel-engine' ); ?></td>
									<td widht="50%"><?php echo wte_get_formated_price( +$sum, $currency, '', ! 0 ); ?></td>
								</tr>
							</table>
						</td>
					</tr>
					...
					<td width="50%">
						<table width="100%">
							<tr>
								...
								<td class="alignright"><?php echo wte_get_formated_price( +$cart_info['subtotal'], $currency, '', ! 0 ); ?></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td class="alignright"><?php echo wte_get_formated_price( $cart_info['total'], $currency, '', ! 0 ); ?></td>
							</tr>
						</table>
					</td>
				</tr>
	...
	public function to( $to ) {
		...
		if ( in_array( $to, array( 'customer', 'admin' ) ) ) {
			switch ( $to ) {
				case 'admin':
					if ( wte_array_get( $this->settings, 'disable_notif', false ) !== '1' ) {
