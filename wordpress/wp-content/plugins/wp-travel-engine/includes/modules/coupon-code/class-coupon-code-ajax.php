...
class Ajax {
	public function __construct() {
		add_action( 'wp_ajax_wp_travel_engine_check_coupon_code', array( __CLASS__, 'check_coupon_code' ) );

		// Apply Coupon AJAX
		add_action( 'wp_ajax_wte_session_cart_apply_coupon', array( __CLASS__, 'apply_coupon' ) );
		add_action( 'wp_ajax_nopriv_wte_session_cart_apply_coupon', array( __CLASS__, 'apply_coupon' ) );

		add_action( 'wp_ajax_wte_session_cart_reset_coupon', array( __CLASS__, 'reset_coupon' ) );
		add_action( 'wp_ajax_nopriv_wte_session_cart_reset_coupon', array( __CLASS__, 'reset_coupon' ) );
	}
	...
	public static function check_coupon_code() {
		...
		$post_id   = intval( sanitize_text_field( $_POST['coupon_id'] ) );
		$coupon_id = CouponCode::coupon_id_by_code( $_POST['coupon_code'] );
	...
	public static function apply_coupon() {
		...
		$coupon_id = CouponCode::coupon_id_by_code( $_POST['CouponCode'] );
		...
		$trip_ids = json_decode( wp_unslash( $_POST['trip_ids'] ) );
		...
		if ( ! ! $coupon_limit_number && ( +$coupon_limit_number <= CouponCode::get_usage_count( $coupon_id ) ) ) {
			\wp_send_json_error(
				new \WP_Error( 'WTE_COUPON_INVALID', sprintf( __( 'Coupon "%1$s" has expired. Maximum no. of coupon usage exceeded.', 'wp-travel-engine' ), $_POST['CouponCode'] ) )
			);
			die;
		}
		...
		if ( 'fixed' == $discount_type ) {
			if ( $discount_value >= $cart_total ) {
				\wp_send_json_error(
					new \WP_Error( 'WTE_COUPON_AMOUNT_EXCEED', sprintf( __( 'Coupon "%1$s" cannot be applied for this trip.', 'wp-travel-engine' ), $_POST['CouponCode'] ) )
				);
				die;
			}
		...
		$wte_cart->add_discount_values( $_POST['CouponCode'], $discount_type, $discount_value );
		// if ( $wte_cart->add_discount_values( $_POST['CouponCode'], $discount_type, $discount_value ) ) {
		...
		wp_send_json_success(
			array(
				...
				'type'                => 'success',
				'message'             => sprintf( __( 'Coupon "%1$s" applied successfully.', 'wp-travel-engine' ), $_POST['CouponCode'] ), // phpcs:ignore
			)
		);
		die;
	}
