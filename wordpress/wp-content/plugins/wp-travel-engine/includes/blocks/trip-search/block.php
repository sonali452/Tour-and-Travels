...
<div class="wpte-gblock-wrapper">
	...
	if ( $show_title || $show_subtitle ) :
		...
		<div class="wpte-gblock-title-wrap">
			...
			if ( $show_title ) :
				if ( $title_level ) {
					printf( '<h%1$d>%2$s</h%1$d>', $title_level, $attributes->{'title'} );
				} else {
					printf( '<p>%1$s</p>', $attributes->{'title'} );
				}
			endif;
			?>
			<?php
			if ( $show_subtitle ) :
				printf( '<p>%2$s</p>', $title_level, $attributes->{'subtitle'} );
			endif;
			?>
		</div>
	<?php endif; ?>
	<div class="wpte-trip-sfilter-wrapper">
		<form class="wpte-trip__search-fields<?php $is_horizontal && print( ' horizontal' ); ?>" action="/trip-search-result">
			<?php
			$icons = [
