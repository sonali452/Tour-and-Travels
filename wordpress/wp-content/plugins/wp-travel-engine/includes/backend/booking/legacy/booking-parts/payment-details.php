...
<div class="wpte-block wpte-col3">
	...
	<div class="wpte-block-content">
		...
		if ( ! empty( $additional_fields ) ) {
			...
			?>
			<h4 class="wpte-title"><?php echo $partial_label . ' ' . __( 'Payment Details', 'wp-travel-engine' ); ?></h4>
			<ul class="wpte-list">
				<?php
				...
				if ( isset( $booking_meta['place_order']['cost'] ) && ! empty( $booking_meta['place_order']['cost'] ) ) {
					...
					<li>
						...
						if ( isset( $booking_meta['place_order']['partial_cost'] ) && ! empty( $booking_meta['place_order']['partial_cost'] ) ) {
							echo wte_get_formated_price( $booking_meta['place_order']['partial_cost'] );
						} elseif ( isset( $booking_meta['place_order']['cost'] ) && ! empty( $booking_meta['place_order']['cost'] ) ) {
							echo wte_get_formated_price( $booking_meta['place_order']['cost'] );
						}
						?>
						</span>
					</li>
		...
		if ( ! empty( $remaining_payment_detail ) ) {
			?>
			<h4 class="wpte-title"><?php _e( 'Remaining Payment Details', 'wp-travel-engine' ); ?></h4>
			<ul class="wpte-list">
			<?php
			...
			if ( isset( $booking_meta['place_order']['partial_due'] ) && ! empty( $booking_meta['place_order']['partial_due'] ) ) {
				?>
					<li>
						<b><?php _e( 'Remaining Amount Paid', 'wp-travel-engine' ); ?></b>
						<span>
						<?php
						if ( isset( $booking_meta['place_order']['partial_due'] ) && ! empty( $booking_meta['place_order']['partial_due'] ) ) {
							echo wte_get_formated_price_html( $booking_meta['place_order']['partial_due'] );
						}
						?>
						</span>
					</li>
