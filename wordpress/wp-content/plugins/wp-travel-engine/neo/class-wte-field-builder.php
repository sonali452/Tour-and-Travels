	...
	public function field() {
			...
			case 'select':
				...
				<select <?php echo $this->get_attributes(); ?>>
					...
					foreach ( $this->options as $value => $option_args ) :
						...
						?>
						<option <?php echo $option_attributes; ?> <?php echo selected( $value, $this->value ); ?> value="<?php echo esc_attr( $value ); ?>"><?php echo esc_html( $option_args['label'] ); ?></option>
					<?php endforeach; ?>
				</select>
				<?php
				$field = ob_get_clean();
		...
		<div class="<?php echo esc_attr( $wrapper_classes ); ?>">
			...
			if ( $this->label && ! in_array( $this->type, array( 'checkbox', 'custom', 'radio' ) ) ) {
				echo "<label class=\"wpte-field-label\" for=\"{$this->id}\">{$this->label}</label>";
			}
			echo isset( $this->before_field ) ? $this->before_field : '';
			echo $field;
			echo isset( $this->after_field ) ? $this->after_field : '';
			if ( $this->tooltip ) {
				echo "<span class=\"wpte-tooltip\">{$this->tooltip}</span>";
			}
			?>
		</div>
		<?php
