	...
	public function init_hooks() {
		...
		add_action(
			...
			function( $post_id, $post ) {
				...
				if ( isset( $_REQUEST['wp_travel_engine_booking_setting']['place_order'] ) ) {
		...
		add_action(
			'init',
			function() {
				if ( defined( 'DOING_AJAX' ) && isset( $_REQUEST['action'] ) && 'wte_session_cart_apply_coupon' === $_REQUEST['action'] && class_exists( 'WP_Travel_Engine_Coupons' ) ) {
					$instance = \WP_Travel_Engine_Coupons::instance();
					if ( version_compare( $instance->version, '1.0.4', '<=' ) ) {
						wp_send_json_error(
							new \WP_Error(
								'WTE_COUPON_ERROR',
				...
				if ( isset( $_REQUEST['wp_travel_engine_placeorder_setting'] ) ) {
					global $wte_cart;
