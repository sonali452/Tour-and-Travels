		...
		foreach ( $order_trips as $key => $item ) :
			...
			<div class="detail-item">
				...
				foreach ( $item->pax as $category_id => $tcount ) {
					...
					echo "{$tcount} X {$label} (" . wte_get_formated_price( $pax_cost, $currency, '', ! 0 ) . ') = ' . wte_get_formated_price( $item->pax_cost[ $category_id ], $currency, '', ! 0 );
					echo '<br/>';
				}
				?>
				</span>
		...
		if ( is_array( $booking->payments ) && count( $booking->payments ) ) {
			foreach ( $booking->payments as $index => $pid ) {
				...
				<div class="detail-item">
					...
					<span class="value"><?php echo esc_html( wte_get_formated_price( $payment_amount, '', '', false, false, true ) ) . " <code>[{$payment_obj->payment_status}]</code>"; ?></span>
				</div>
				<?php
			}
		}
