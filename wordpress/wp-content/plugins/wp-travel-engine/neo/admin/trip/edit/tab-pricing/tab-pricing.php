	...
	array(
		10 => array(
			...
			'content_callback' => function() {
				require_once plugin_dir_path( WP_TRAVEL_ENGINE_FILE_PATH ) . 'neo/admin/trip/edit/tab-pricing/tab-pricing__packages.php';
			},
		),
		20 => array(
			'content_id'       => 'partial-payment-upsell',
			'content_callback' => function() {
					require_once plugin_dir_path( WP_TRAVEL_ENGINE_FILE_PATH ) . 'neo/admin/trip/edit/tab-pricing/tab-pricing__partial-payment-upsell.php';
			},
		),
	),
	$post_ID
...
if ( $next_tab ) :
	...
	<div class="wpte-field wpte-submit">
		<input data-tab="pricing" data-post-id="<?php echo esc_attr( $post_ID ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'wpte-trip-tab-save-continue' ) ); ?>" data-next-tab="<?php echo isset( $next_tab['callback_function'] ) ? esc_attr( $next_tab['callback_function'] ) : ''; ?>" class="wpte_save_continue_link" type="submit" name="wpte_trip_tabs_save_continue" value="<?php _e( 'Continue', 'wp-travel-engine' ); ?>">
	</div>
	<?php
endif;
