...
class TripSearch {
	...
	private function init_hooks() {
		...
		// Ajax Handler.
		add_action( 'wp_ajax_wte_show_ajax_result', array( __CLASS__, 'filter_trips_html' ) );
		add_action( 'wp_ajax_nopriv_wte_show_ajax_result', array( __CLASS__, 'filter_trips_html' ) );
		add_action( 'wp_ajax_wte_show_ajax_result_load', array( __CLASS__, 'load_trips_html' ) );
		add_action( 'wp_ajax_nopriv_wte_show_ajax_result_load', array( __CLASS__, 'load_trips_html' ) );
	...
	public static function filter_trips_html() {
		...
		$view_mode  = ! empty( $_REQUEST['mode'] ) ? $_REQUEST['mode'] : wp_travel_engine_get_archive_view_mode();
		$view_class = 'grid' === $view_mode ? 'col-2 category-grid' : 'category-list';
		...
		if ( $query->found_posts > $default_posts_per_page ) {
			echo "<span data-id='" . $query->found_posts . "' class='wte-search-load-more'><a data-query-vars='" . json_encode( $query->query_vars ) . "' data-current-page='" . $paged . "' data-max-page='" . $query->max_num_pages . "' href='#' class='load-more-search'>" . __( 'Load More', 'wp-travel-engine' ) . '</a></span>';
		}
	...
	public static function get_query_args( $ajax_request = false ) {
		...
		if ( ! empty( $_REQUEST['trip-date-select'] ) || ! empty( $_REQUEST['date'] ) ) {
			$date = $ajax_request ? $_REQUEST['date'] : $_REQUEST['trip-date-select'];
		...
		if ( isset( $_REQUEST['sort'] ) && ! empty( $_REQUEST['sort'] ) ) {
			$sortby_val = isset( $_REQUEST['sort'] ) && ! empty( $_REQUEST['sort'] ) ? $_REQUEST['sort'] : 'menu_order';
		...
		if ( ! empty( $_REQUEST['wte_orderby'] ) || ! empty( $_REQUEST['sort'] ) ) {
			$order_by   = $ajax_request ? $_REQUEST['sort'] : $_REQUEST['wte_orderby'];
	...
	public static function get_filters_sections() {
		...
		if ( ! empty( $settings['trip_search']['apply_in_search_page'] ) && '1' === $options['trip_search']['apply_in_search_page'] ) {
			return array();
		}
		...
		if ( is_array( $terms ) && count( $terms ) > 0 ) {
			...
			foreach ( $terms as $term ) {
				...
				printf(
					...
					. '<span>%4$s</span>'
					. '</label>',
					checked( $term->slug, wte_array_get( $_GET, $term->taxonomy, false ), false ),
					$term->slug,
					$term->taxonomy,
					$term->name
				);
	...
	public static function filter_taxonomies_render( $taxonomy, $filter ) {
		...
		echo "<div class='advanced-search-field search-trip-type'>
		<h3 class='filter-section-title trip-type'>" . $filter['label'] . '</h3>';
		// $categories_sorted = array();
		// wte_advanced_search_sort_terms_hierarchicaly( $categories, $categories_sorted );
		echo '<div class="filter-section-content">';
		self::taxonomy_filter_html( $terms );
		// wte_advanced_search_taxonomy_render( $categories_sorted, false, 'destination' );
		echo '</div></div>';
	}
	...
	public static function choose_page( $pages ) {
		...
		$pages['wte-advamced-search-page'] = array(
