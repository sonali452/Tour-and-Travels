	...
	<div class="category-trips-single-inner-wrap">
		<figure class="category-trip-fig">
			...
			<?php if ( wte_array_get( $settings, 'layoutFilters.showFeaturedImage', true ) ) : ?>
				<a href="<?php echo get_the_permalink( $trip ); ?>">
					<?php
			...
			<div class="category-trip-detail-wrap">
					...
					<div class="category-trip-desti">
							...
							<span class="category-trip-dur">
								...
								<span>
									<?php if ( ! empty( $meta->duration['days'] ) ) : ?>
										<span><?php printf( _n( '%d Day', '%d Days', $meta->duration['days'], 'wp-travel-engine' ), $meta->duration['days'] ); ?></span>
									<?php endif; ?>
									<?php if ( false && ! empty( $meta->duration['nights'] ) ) : ?>
										<span><?php printf( _n( '%d Night', '%d Nights', $meta->duration['nights'], 'wp-travel-engine' ), $meta->duration['nights'] ); ?></span>
									<?php endif; ?>
								</span>
							</span>
						<?php endif; ?>
						<?php if ( wte_array_get( $settings, 'layoutFilters.showGroupSize', false ) && (int) $meta->min_pax ) : ?>
							<span class="wpte-trip-meta category-trip-dur wpte-trip-pax">
								...
								<?php printf( '<span>' . __( '%s People', 'wp-travel-engine' ) . '</span>', (int) $meta->max_pax ? $meta->min_pax . '-' . $meta->max_pax : $meta->min_pax ); ?>
							</span>
						<?php endif; ?>
					</div>
								...
								<div class="category-disc-feat-wrap">
									<div class="category-trip-discount">
										<span class="discount-offer">
											<span><?php echo sprintf( __( '%1$s%% ', 'wp-travel-engine' ), $meta->discount_percent ); ?></span>
										<?php _e( 'Off', 'wp-travel-engine' ); ?></span>
									</div>
								</div>
							<?php endif; ?>
							<span class="price-holder">
								<span class="actual-price"><?php echo wte_get_formated_price_html( $meta->has_sale ? $meta->sale_price : $meta->price ); ?></span>
								<?php if ( wte_array_get( $settings, 'layoutFilters.showStrikedPrice', true ) && $meta->has_sale ) : ?>
								<span class="striked-price"><?php echo wte_get_formated_price_html( $meta->price ); ?></span>
								<?php endif; ?>
							</span>
						</div>
					<?php endif; ?>
				...
				<?php if ( \wte_array_get( $settings, 'layoutFilters.showDescription', false ) ) : ?>
					<div class="category-trip-desc">
						<p><?php echo \wte_get_the_excerpt( $trip->ID, wte_array_get( $settings, 'excerptLength', 10 ) ); ?></p>
					</div>
				<?php endif; ?>
				<?php if ( wte_array_get( $settings, 'layoutFilters.showViewMoreButton', true ) && wte_array_get( $settings, 'layout', '' ) == 'grid' ) : ?>
					<a href="<?php echo esc_url( get_the_permalink( $trip->ID ) ); ?>" class="button category-trip-viewmre-btn"><?php echo wte_array_get( $settings, 'viewMoreButtonText', __( 'View Details', 'wp-travel-engine' ) ); ?></a>
				<?php endif; ?>
			</div>
		</div>
					...
					switch ( $dates_layout ) {
						...
						case 'dates_list':
							...
							foreach ( $fsds as $fsd ) {
								...
								printf( '<li><span class="left">%1$s %2$s</span><span class="right">%3$s</span></li>', $icon, wte_get_formated_date( $fsd['start_date'] ), sprintf( __( '%s Available', 'wp-travel-engine' ), $fsd['seats_left'] ) );
							}
							echo '</ul>';
							echo '</div>';
							break;
