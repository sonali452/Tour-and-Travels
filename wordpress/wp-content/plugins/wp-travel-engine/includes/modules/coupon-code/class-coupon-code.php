...
class CouponCode {
	...
	private function init_hooks() {
		...
		add_filter(
			...
			function( $messages ) {
				...
				$messages['wte-coupon'] = array(
					...
					5  => isset( $_GET['revision'] ) ? sprintf( __( '%1$s restored to revision from %2$s', 'wp-travel-engine' ), $post_object->labels->singular_name, wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		...
		add_action( 'save_post', array( __CLASS__, 'save_wte_coupon_metabox' ) );
	...
	public static function save_wte_coupon_metabox( $coupon_id ) {
		...
		empty( $_POST['wp_travel_engine_coupon_code'] ) || update_post_meta( $coupon->ID, 'wp_travel_engine_coupon_code', $_POST['wp_travel_engine_coupon_code'] );
		empty( $_POST['wp_travel_engine_coupon'] ) || update_post_meta( $coupon->ID, 'wp_travel_engine_coupon_metas', wp_unslash( $_POST['wp_travel_engine_coupon'] ) );
