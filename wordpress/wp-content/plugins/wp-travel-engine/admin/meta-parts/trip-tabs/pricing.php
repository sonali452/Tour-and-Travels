...
$default_pricing_options   = apply_filters(
	...
	array(
		...
		'group'  => __( 'Group', 'wp-travel-engine' ),
	)
);
...
if ( $trip && version_compare( $trip->trip_version, '2.0.0', '<' ) ) {
	...
	<div class="wpte-info-block">
		<p>
			...
			?>
				<a href="<?php echo esc_url( $migrate_url ); ?>"><?php echo esc_html( 'Migrate trip', 'wp-travel-engine' ); ?></a>
		</p>
	</div>
	<?php
}
?>
	...
	<?php if ( $next_tab ) : ?>
		<div class="wpte-field wpte-submit">
			<input data-tab="pricing" data-post-id="<?php echo esc_attr( $post_id ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'wpte-trip-tab-save-continue' ) ); ?>" data-next-tab="<?php echo esc_attr( $next_tab['callback_function'] ); ?>" class="wpte_save_continue_link" type="submit" name="wpte_trip_tabs_save_continue" value="<?php _e( 'Save &amp; Continue', 'wp-travel-engine' ); ?>">
		</div>
		<?php
	endif;
