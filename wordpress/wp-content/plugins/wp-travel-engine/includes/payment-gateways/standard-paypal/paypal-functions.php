...
function wp_travel_engine_process_paypal_ipn( $tokenized_data ) {
	...
	if ( $verified = $listener->processIpn() ) {
		...
		file_put_contents( 'ipn_success.log', json_encode( array( 'data' => $transactionData ), JSON_PRETTY_PRINT ) . PHP_EOL, LOCK_EX | FILE_APPEND );
		...
		if ( isset( $_REQUEST['payment_status'] ) ) {
			$payment_meta_input['payment_status'] = strtolower( $_REQUEST['payment_status'] );
			$payment_meta_input['_txn_id']        = $_REQUEST['txn_id'];
		...
		$payment_meta_input['gateway_response'] = $_REQUEST;
