...
class Updates {
	...
	public function __construct() {
		add_action( 'in_plugin_update_message-wp-travel-engine/wp-travel-engine.php', array( $this, 'in_plugin_update_message' ), 10, 2 );
	}
	...
	public function in_plugin_update_message( $file, $plugin_data ) {
		...
		foreach ( $plugins as $file => $plugin ) {
			if ( empty( $plugin[ self::VERSION_TESTED_HEADER ] ) ) {
				$plugin[ self::VERSION_TESTED_HEADER ] = '4.3.7'; // Because header has been added since 5.0.0.
			}

			$plugin_version_parts = explode( '.', $plugin[ self::VERSION_TESTED_HEADER ] );
			$plugin_version       = $plugin_version_parts[0];

			if ( version_compare( $plugin_version . '.0', $version . '.0', '<' ) ) {
				$upgrade_type                    = 'major';
